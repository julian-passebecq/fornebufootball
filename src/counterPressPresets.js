export const COUNTERPRESS_VERSION = 3
  ...
  '9v9': [
    {
      ...
      note: T(
        'Recommended: still connected, but with a little more width so players can recognise their lanes.',
        'Recommandé : le bloc reste connecté, avec un peu plus de largeur pour que les joueurs reconnaissent mieux leurs couloirs.',
        'Anbefalt: laget holder kontakt, men med litt mer bredde slik at spillerne kjenner igjen sonene sine.'
      ),
      positions: {
    ...
    },
    {
      ...
      note: T(
        'More players around the loss. Higher pressure, but less cover if the press is beaten.',
        'Plus de joueurs autour de la perte. Pression plus forte, mais moins de couverture si le pressing est éliminé.',
        'Flere spillere rundt balltapet. Høyere press, men mindre sikring hvis presset blir spilt av.'
      ),
      positions: {
  ...
  '7v7': [
    {
      ...
      note: T(
        'Recommended: compact enough to counter-press, but slightly wider so young players keep clear reference positions.',
        'Recommandé : assez compact pour presser à la perte, mais légèrement plus large pour garder des repères clairs.',
        'Anbefalt: kompakt nok til gjenvinningspress, men litt bredere så unge spillere beholder tydelige referanseposisjoner.'
      ),
      positions: {
    ...
    },
    {
      ...
      note: T(
        'Two players can jump onto the ball and nearest outlet. Safer back three, but the central midfielder works hard.',
        'Deux joueurs peuvent fermer le porteur et la solution proche. Trois derrière sécurisent, mais le milieu axial travaille beaucoup.',
        'To spillere kan gå på ball og nærmeste pasningsalternativ. Tre bak gir sikring, men sentral midtbane må dekke mye.'
      ),
      positions: {
...
function ballLossGlobalSections() {
  return [
    {
      key: 'press',
      label: T('First 5 seconds', 'Les 5 premières secondes', 'De første 5 sekundene'),
      text: T(
        'Closest player attacks the ball immediately. Everyone else squeezes toward the loss.',
        'Le joueur le plus proche attaque immédiatement le ballon. Tous les autres resserrent autour de la perte.',
        'Nærmeste spiller går på ball med en gang. Alle andre trekker inn mot balltapet.'
      ),
    },
    withoutBallSection(),
    {
      ...
      text: T(
        'Close inside passing lanes first. Force play outside or backwards.',
        'Fermer d’abord les passes intérieures. Forcer le jeu vers l’extérieur ou vers l’arrière.',
        'Steng pasninger gjennom sentrum først. Tving spillet ut eller bakover.'
      ),
    },
    {
      key: 'transition',
      ...
      text: T(
        'Second and third players arrive behind the press. The back line steps up to keep useful supporting distances.',
        'Le deuxième et le troisième joueurs arrivent derrière le pressing. La ligne arrière avance pour garder des distances de soutien utiles.',
        'Andre og tredje spiller kommer bak presset. Forsvarslinjen skyver opp for å holde gode støtteavstander.'
      ),
    },
    {
      key: 'fallback',
      ...
      text: T(
        'Stop chasing and recover centrally into the team block.',
        'Arrêter de poursuivre et se recentrer dans le bloc équipe.',
        'Slutt å jage og fall inn sentralt i lagblokken.'
      ),
    },
  ]
}

function syncSharedPlayerInfo(standardPlan, ballLossPlan) {
  if (!standardPlan?.players || !ballLossPlan?.players) return
  for (const standardPlayer of standardPlan.players) {
    const ballLossPlayer = ballLossPlan.players.find(player => player.number === standardPlayer.number)
    if (!ballLossPlayer) continue
    ballLossPlayer.role = clone(standardPlayer.role)
    ballLossPlayer.roleBand = standardPlayer.roleBand
    ballLossPlayer.sections = clone(standardPlayer.sections || [])
  }
}

function ensureCounterPressCopy(plan, standardPlan) {
  if (!plan?.global) return
  plan.global.title = clone(BALL_LOSS_TITLE)
  const existing = Object.fromEntries((plan.global.sections || []).map(section => [section.key, section]))
  plan.global.sections = ballLossGlobalSections().map(section => existing[section.key] ? {
    ...section,
    text: { ...section.text, ...(existing[section.key].text || {}) },
  } : section)
  syncSharedPlayerInfo(standardPlan, plan)
}
...
export function createCounterPressPlan(standardPlan, formation, presetId = DEFAULT_COUNTERPRESS_PRESET[formation]) {
  ...
  plan.global = {
    ...
    sections: ballLossGlobalSections(),
  }
...
export function prepareCounterPressData(source) {
  ...
  for (const formation of ['7v7', '9v9']) {
    ...
    // Individual player guidance is one shared page, independent of the tactical phase.
    // Only player coordinates change between Starting formation and On ball loss.
    ensureCounterPressCopy(profile.tactics.alternative, profile.tactics.standard)
  ...
  next.version = Math.max(Number(next.version || 0), 13)
  return next
}
