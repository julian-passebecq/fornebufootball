...
import {
  ...
  getCounterPressPreset,
  prepareCounterPressData,
} from './counterPressPresets'
import './styles.css'
import './v5.css'
import './v6.css'
import './v8.css'
...
const labels = {
  en: {
    title: 'Fornebu Coach Board', subtitle: 'Match plan and player roles', club: 'Fornebu FK', teamPlan: 'Team plan', playerPlan: 'Player plan', standard: 'Standard', alternative: 'Counter-press', password: 'Coach password', unlock: 'Sign in', editHint: 'Drag players and edit short points. Standard and Counter-press positions are saved separately.', save: 'Publish changes', saving: 'Publishing…', saved: 'Published', saveError: 'Could not publish – try again', selectPlayer: 'Click a player for the individual plan', backTeam: 'Back to team plan', position: 'Position', attack: 'Attack', defend: 'Defend', transition: 'Transition', keyCue: 'Key cue', formation: 'Formation', loginError: 'Wrong password or the coach server is not configured.', publicPlans: 'Show to players', shown: 'Shown', hidden: 'Hidden', visibilityHint: 'Choose which plans players can see. At least one plan must remain visible.', coachWorkspace: 'Coach workspace', publicBoard: 'Player board', signOut: 'Sign out', editPoints: 'Use one short point per line.', editingPlan: 'Editing', planPositions: 'Positions and text are separate for this plan.', compactRule: 'Red team rule', inRule: 'IN principle', outRule: 'OUT principle', privateArea: 'Private editing area for the coach.', counterShape: 'Counter-press shape', presetHint: 'Choose a starting counter-press shape. You can still drag players afterwards.', recommended: 'Recommended'
  },
  no: {
    title: 'Fornebu trenerbrett', subtitle: 'Kampplan og spillerroller', club: 'Fornebu FK', teamPlan: 'Lagplan', playerPlan: 'Spillerplan', standard: 'Standard', alternative: 'Gjenvinningspress', password: 'Trenerpassord', unlock: 'Logg inn', editHint: 'Dra spillere og rediger korte punkter. Standard og gjenvinningspress lagres separat.', save: 'Publiser endringer', saving: 'Publiserer…', saved: 'Publisert', saveError: 'Kunne ikke publisere – prøv igjen', selectPlayer: 'Klikk en spiller for individuell plan', backTeam: 'Tilbake til lagplan', position: 'Posisjon', attack: 'Angrep', defend: 'Forsvar', transition: 'Overgang', keyCue: 'Nøkkelpunkt', formation: 'Formasjon', loginError: 'Feil passord eller trener-serveren er ikke konfigurert.', publicPlans: 'Vis for spillerne', shown: 'Vises', hidden: 'Skjult', visibilityHint: 'Velg hvilke planer spillerne skal se. Minst én plan må være synlig.', coachWorkspace: 'Trenerrom', publicBoard: 'Spillerside', signOut: 'Logg ut', editPoints: 'Ett kort punkt per linje.', editingPlan: 'Redigerer', planPositions: 'Posisjoner og tekst er egne for denne planen.', compactRule: 'Rød lagregel', inRule: 'IN-prinsipp', outRule: 'OUT-prinsipp', privateArea: 'Privat redigeringsside for treneren.', counterShape: 'Form for gjenvinningspress', presetHint: 'Velg en startform. Du kan fortsatt dra spillerne etterpå.', recommended: 'Anbefalt'
  },
  fr: {
    title: 'Tableau Coach Fornebu', subtitle: 'Plan de match et rôles des joueurs', club: 'Fornebu FK', teamPlan: "Plan d'équipe", playerPlan: 'Plan joueur', standard: 'Standard', alternative: 'Contre-pressing', password: 'Mot de passe coach', unlock: 'Connexion', editHint: 'Déplacez les joueurs et modifiez les consignes courtes. Standard et contre-pressing sont enregistrés séparément.', save: 'Publier les modifications', saving: 'Publication…', saved: 'Publié', saveError: 'Publication impossible – réessayez', selectPlayer: 'Cliquez sur un joueur pour voir son plan individuel', backTeam: "Retour au plan d'équipe", position: 'Position', attack: 'Attaque', defend: 'Défense', transition: 'Transition', keyCue: 'Point clé', formation: 'Formation', loginError: 'Mot de passe incorrect ou serveur coach non configuré.', publicPlans: 'Afficher aux joueurs', shown: 'Visible', hidden: 'Masqué', visibilityHint: 'Choisissez les plans visibles par les joueurs. Au moins un plan doit rester visible.', coachWorkspace: 'Espace coach', publicBoard: 'Vue joueurs', signOut: 'Déconnexion', editPoints: 'Une consigne courte par ligne.', editingPlan: 'Modification', planPositions: 'Les positions et les textes sont indépendants pour ce plan.', compactRule: "Règle d'équipe en rouge", inRule: 'Principe IN', outRule: 'Principe OUT', privateArea: 'Espace privé de modification pour le coach.', counterShape: 'Forme de contre-pressing', presetHint: 'Choisissez une forme de départ. Vous pouvez ensuite déplacer les joueurs.', recommended: 'Recommandé'
  },
}
...
function CounterPressPresetEditor({ formation, selectedPreset, lang, t, onSelect }) {
  const presets = COUNTERPRESS_PRESETS[formation] || []
  return <section className="counterpress-preset-editor">
    <div className="counterpress-preset-head"><span>{t.counterShape}</span><small>{t.presetHint}</small></div>
    <div className="counterpress-preset-options">
      {presets.map(preset => <button key={preset.id} type="button" className={selectedPreset === preset.id ? 'active' : ''} onClick={() => onSelect(preset.id)}>
        <div><strong>{localized(preset.name, lang)}</strong><span>{preset.shape}</span></div>
        {preset.recommended && <em>{t.recommended}</em>}
        <small>{localized(preset.note, lang)}</small>
      </button>)}
    </div>
  </section>
}
...
export default function AppV12() {
  ...
  const [data, setData] = useState(() => prepareCounterPressData(migrateRemote(seedData)))
  ...
  useEffect(() => {
    ...
    fetch('/api/tactics')
      ...
      .then(remote => { if (active && remote?.['7v7'] && remote?.['9v9']) setData(prepareCounterPressData(migrateRemote(remote))) })
      .catch(() => {})
    return () => { active = false }
  }, [])
  ...
  const baseShape = formationTemplate?.shape || (formation === '7v7' ? '2-3-1' : '3-2-3')
  const counterPreset = getCounterPressPreset(formation, profile?.counterPressPreset)
  const activeShape = strategy === 'alternative' ? (counterPreset?.shape || baseShape) : baseShape
  ...
  function chooseCounterPressPreset(presetId) {
    setData(previous => {
      const next = clone(previous)
      const profileNext = next.formats[formation]
      profileNext.counterPressPreset = presetId
      applyCounterPressPreset(profileNext.tactics.alternative, formation, presetId)
      next.version = 11
      return next
    })
    setSelected(null)
  ...
  return <div className={`app-shell v6-shell v9-shell v10-shell v12-shell ${coachMode ? 'coach-shell' : 'public-shell'}`}>
    ...
    <main><div className="workspace-grid">
      <section className="board-card v6-board-card v9-board-card v10-board-card">
        <div className="board-heading v9-board-heading"><div className="board-format-pill"><strong>{formation}</strong></div><div className={`formation-center-badge ${strategy === 'alternative' ? 'counterpress-shape' : ''}`}><span>{t.formation}</span><strong>{activeShape}</strong></div><div className="board-heading-right"><span className={`active-plan-pill ${strategy}`}>{planLabel}</span></div></div>
      ...
      <aside className="strategy-sidebar v6-strategy-sidebar v9-strategy-sidebar">
        <div className="sidebar-brand-row v9-sidebar-brand-row"><strong>{formation} · {activeShape}</strong></div>
        ...
        {coachMode && strategy === 'alternative' && <CounterPressPresetEditor formation={formation} selectedPreset={profile.counterPressPreset} lang={lang} t={t} onSelect={chooseCounterPressPreset}/>}        
