...
export default function AppV6() {
  ...
  return <div className={`app-shell v6-shell ${coachMode ? 'coach-shell' : 'public-shell'}`}>
    <header className="topbar v5-topbar v6-topbar">
      <a className="brand club-brand" href={CLUB_URL} target="_blank" rel="noreferrer" aria-label="Fornebu FK club website">
        <AppMark className="brand-mark-image"/>
        <div><h1>{t.title}</h1><span><strong>{t.club} ↗</strong> · {coachMode ? t.coachWorkspace : t.subtitle}</span></div>
      </a>

      <div className="team-ribbon v5-team-ribbon v6-team-ribbon" aria-label="Team selection">
        <div className="team-tabs v5-team-tabs v6-team-tabs">
          {teams.map(team => <button key={team.id} className={selectedTeam.id === team.id ? 'active' : ''} onClick={() => chooseTeam(team)}>
            <TeamNumber number={team.number}/>
            <span className="team-tab-label"><strong>{team.name}</strong></span>
          </button>)}
        </div>
        <div className="next-game-ribbon v5-next-game v6-next-game">
    ...
    <main>
      <div className="workspace-grid">
        <section className="board-card v6-board-card">
          <div className="board-heading v5-board-heading v6-board-heading">
            <div className="board-team-meta">
              <span className="eyebrow">{selectedTeam.name}</span>
              <h2>{formation}</h2>
            </div>
            ...
            <div className="board-heading-right">
              ...
              <div className="board-hint">{coachMode ? t.planPositions : t.selectPlayer}</div>
            </div>
          </div>
        ...
        <aside className="strategy-sidebar v6-strategy-sidebar">
          ...
          {!player ? <>
            ...
            <div className="player-sidebar-head">
              <div className="player-identity"><div className="sidebar-jersey v6-sidebar-jersey" style={{ '--player-color': selectedColour }}><span>{player.number}</span></div><div><span className="eyebrow">{t.playerPlan}</span><h2>#{player.number}</h2><div className="role-line"><span>{t.position}</span><strong>{player.role[lang]}</strong></div></div></div>
